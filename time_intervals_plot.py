...
def calculate_time_intervals(pcap_file):
    ...
    packets = packets[10:500]  # Take packets 10 to 500
    time_intervals = []
    packet_numbers = []
    last_time = None
...
fig = plt.figure(figsize=(12, 6 * len(pcap_files)))
grid = plt.GridSpec(len(pcap_files), 1, height_ratios=[1] * len(pcap_files), wspace=0.4, hspace=0.9)
...
fig.text(0.5, 0.05, 'Packet Number', ha='center', fontsize=14)
